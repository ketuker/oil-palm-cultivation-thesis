            		...
            		if (!empty($landp_ag)){
            			...
            			if (!empty($accessibility_ag)){
            				...
            				if (!empty($factors_ag)){
                                ...
                                // print_r($result_geom_from_wkt[0]['st_area']);
                                // die;

                                $limit_area = $result_geom_from_wkt[0]['st_area'];
                                ...
                                $result_data_twn             = substr($result_geojson_twn, 0, -3) . ' ]}';
                                    ...
                                    \Yii::$app->getSession()->setFlash('error', '<div class="alert alert-danger" role="alert">area can not exceed 20,000 hectares!</div>');
                    ...
                    if($epsg !== null) {
                        ...
                        if($jj == 0) {
                            ...
                            if ($geom_type == 'MULTIPOLYGON') {
                                ...
                                $area_of_interest       = "SELECT ST_AsGeoJson(ST_GeomFromText('".$geom_Json."', 4326))";
                                $aoi                    = Yii::$app->db->createCommand($area_of_interest)->queryAll();
                                        ...
                                        if (!empty($landp_ag)){
                                            ...
                                            if (!empty($accessibility_ag)){
                                                ...
                                                if (!empty($factors_ag)){
                                                    ...
                                                    /* Convert WKT to Geom */
                                                    $query_geom_from_wkt    = "SELECT ST_GeomFromText('".$geom_Json."', 4326), (ST_Area(ST_Transform(ST_GeomFromText('".$geom_Json."', 4326),32750))* 0.0001) as st_area";
                                                    $result_geom_from_wkt   = Yii::$app->db->createCommand($query_geom_from_wkt)->queryAll();
                                                ...
                                                if ($limit_area < 20000){
                                                    ...
                                                    /* Climate Rainfall */
                                                    $get_rainfall           = "SELECT b.scorech as scorech,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$geom_Json."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom) = true) as b group by b.scorech;";
                                                    $value_rainfall         = Yii::$app->db->createCommand($get_rainfall)->queryAll();      
                                                    $result_geojson_rain    = '{"type": "FeatureCollection","features": [';
                                                    ...
                                                    /* Climate Temperature */
                                                    $get_temperature            = "SELECT b.scoresuhu as scoresuhu, ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$geom_Json."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom) = true) as b group by b.scoresuhu;";
                                                    $value_temperature          = Yii::$app->db->createCommand($get_temperature)->queryAll();      
                                                    $result_geojson_temp        = '{"type": "FeatureCollection","features": [';
                                                    ...
                                                    /* Climate Dry Month */
                                                    $get_drymonth          = "SELECT b.scoredry as scoredry ,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$geom_Json."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom) = true) as b group by b.scoredry;";
                                                    $value_drymonth        = Yii::$app->db->createCommand($get_drymonth)->queryAll();      
                                                    $result_geojson_dm     = '{"type": "FeatureCollection","features": [';
                                                    ...
                                                    /* Land slope */
                                                    $get_slope                     = "SELECT b.score_lrg as score_lrg ,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$geom_Json."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom) = true) as b group by b.score_lrg order by b.score_lrg asc;";
                                                    $value_slope                   = Yii::$app->db->createCommand($get_slope)->queryAll();      
                                                    $result_geojson_slp            = '{"type": "FeatureCollection","features": [';
                                                    ...
                                                    /* Land  Non Peat Texture */
                                                    $get_texture                   = "SELECT b.score_text as score_text ,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$geom_Json."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom) = true) as b group by b.score_text;";
                                                    $value_texture                 = Yii::$app->db->createCommand($get_texture)->queryAll();      
                                                    $result_geojson_txt            = '{"type": "FeatureCollection","features": [';
                                                    ...
                                                    /* Land Non Peat Elevation */
                                                    $get_elevation                 = "SELECT b.score_elev as score_elev ,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$geom_Json."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom) = true) as b group by b.score_elev;";
                                                    $value_elevation               = Yii::$app->db->createCommand($get_elevation)->queryAll();      
                                                    $result_geojson_elev           = '{"type": "FeatureCollection","features": [';
                                                    ...
                                                    /* Land Peat Thickness */
                                                    $get_thickness                 = "SELECT b.scoredept as scoredept ,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$geom_Json."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom) = true) as b group by b.scoredept;";
                                                    $value_thickness               = Yii::$app->db->createCommand($get_thickness)->queryAll();      
                                                    $result_geojson_thick          = '{"type": "FeatureCollection","features": [';
                                                    ...
                                                    /* Accessibility Distance From Road */
                                                    $get_road           = "SELECT b.score_road as score_road ,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$geom_Json."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom) = true) as b group by b.score_road;";
                                                    $value_road         = Yii::$app->db->createCommand($get_road)->queryAll();      
                                                    $result_geojson_rod     = '{"type": "FeatureCollection","features": [';
                                                    for ($i=0; $i < count($value_road); $i++) { 
                                                        $score_road                    = $value_road[$i]['score_road'];
                                                    ...
                                                    /* Accessibility Distance From Mills */
                                                    $get_mills           = "SELECT b.score_mill as score_mill ,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$geom_Json."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom) = true) as b group by b.score_mill;";
                                                    $value_mills         = Yii::$app->db->createCommand($get_mills)->queryAll();      
                                                    $result_geojson_mls     = '{"type": "FeatureCollection","features": [';
                                                    for ($i=0; $i < count($value_mills); $i++) { 
                                                        $score_mill                    = $value_mills[$i]['score_mill'];
                                                    ...
                                                    /* Accessibility Distance From town */
                                                    $get_town           = "SELECT b.score_town as score_town ,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$geom_Json."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$geom_Json."', 4326),u.geom) = true) as b group by b.score_town;";
                                                    $value_town         = Yii::$app->db->createCommand($get_town)->queryAll();      
                                                    $result_geojson_twn     = '{"type": "FeatureCollection","features": [';
