...
use app\models\SensitivitySearch;
...
/**
 * SensitivityController implements the CRUD actions for Sensitivity model.
 */
class SensitivityController extends Controller
{
    ...
    /**
     * Lists all Sensitivity models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new SensitivitySearch();
        ...
        public function actionNodata()
    {
        ...
        $advusr = '';   
        $adv = new Advusr();
        $get_adv       = "SELECT * from advusr";
        $value_adv    = Yii::$app->db->createCommand($get_climag)->queryAll();  

            if ($value_adv== null){
                $advusr = "You didn't have any Sensitivity Analysis Yet";         
            }
    ...
    /**
     * Creates a new Sensitivity model.
    ...
    public function actionCreatedraw()
    {
        ...
        if (Yii::$app->request->post()) {
             ...
             * B = Hasil intersects
             **/
            $get_intersection       = "SELECT b.*, ST_AsGeojson(ST_intersection(b.geom,ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326))) as geom from (select kesesuaian.*, st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),kesesuaian.geom)as touch from kesesuaian where st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),kesesuaian.geom) = true) as b;";
            $value_intersections    = Yii::$app->db->createCommand($get_intersection)->queryAll();      

            $area_of_interest       = "SELECT ST_AsGeoJson(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326))";
            $aoi                    = Yii::$app->db->createCommand($area_of_interest)->queryAll();

            /***
             * Get Average Geometry Climate
             **/
            $climate_ag             = Advusr::find()->OrderBy('id DESC')->one();
            $landnp_ag             = Advusr::find()->OrderBy('id DESC')->one();
            $landp_ag             = Advusr::find()->OrderBy('id DESC')->one();
            $accessibility_ag             = Advusr::find()->OrderBy('id DESC')->one();

            /***
             * Get Average Geometry Factor
             **/
            $factors_ag             = Advusr::find()->OrderBy('id DESC')->one();
                        ...
                        if (isset($accessibility_ag)){
                            if (isset($factors_ag)){
                                ...
                                for ($i=0; $i < count($value_intersections); $i++) { 
                                    $kesesuaian_climate         = ($value_intersections[$i]['scorech'] * ($climate_ag->bobot_ch * $factors_ag->bobot_climate)) + ($value_intersections[$i]['scoresuhu'] * ($climate_ag->bobot_temp * $factors_ag->bobot_climate)) + ($value_intersections[$i]['scoredry'] * ($climate_ag->bobot_dm * $factors_ag->bobot_climate));
                                    $status_climate             = 'N';
                                ...
                                /* Convert WKT to Geom */
                                $query_geom_from_wkt    = "SELECT ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326), ST_Area(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326))";
                                ...
                                $get_rainfall           = "SELECT b.scorech as scorech,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom) = true) as b group by b.scorech;";
                                ...
                                $get_temperature            = "SELECT b.scoresuhu as scoresuhu, ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom) = true) as b group by b.scoresuhu;";
                                ...
                                $get_drymonth           = "SELECT b.scoredry as scoredry ,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom) = true) as b group by b.scoredry;";
                                ...
                                $get_slope            = "SELECT b.score_lrg as score_lrg ,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom) = true) as b group by b.score_lrg order by b.score_lrg asc;";
                                ...
                                $get_texture           = "SELECT b.score_text as score_text ,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom) = true) as b group by b.score_text;";
                                ...
                                $get_elevation           = "SELECT b.score_elev as score_elev ,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom) = true) as b group by b.score_elev;";
                                ...
                                $get_thickness           = "SELECT b.scoredept as scoredept ,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom) = true) as b group by b.scoredept;";
                                ...
                                $get_road           = "SELECT b.score_road as score_road ,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom) = true) as b group by b.score_road;";
                                ...
                                $get_mills           = "SELECT b.score_mill as score_mill ,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom) = true) as b group by b.score_mill;";
                                ...
                                $get_town           = "SELECT b.score_town as score_town ,ST_AsGeojson(st_union(st_intersection(b.geom,ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326))))as geom from (select u.*, st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom)as touch from  kesesuaian as u where st_intersects(ST_GeomFromText('".$_POST['Sensitivity']['geom']."', 4326),u.geom) = true) as b group by b.score_town;";
            ...
            $_POST['Sensitivity']['data']       = $result_data;
            $_POST['Sensitivity']['geom']       = $result_geom_from_wkt[0]['st_geomfromtext'];
            $_POST['Sensitivity']['st_area']    = $data_st_area;
            $_POST['Sensitivity']['data_rain']  = $result_data_rain;
            $_POST['Sensitivity']['data_temp']  = $result_data_temp;
            $_POST['Sensitivity']['data_dm']  = $result_data_dm;
            $_POST['Sensitivity']['data_slope']  = $result_data_slp;
            $_POST['Sensitivity']['data_text']  = $result_data_txt;
            $_POST['Sensitivity']['data_elev']  = $result_data_elev;
            $_POST['Sensitivity']['data_thick']  = $result_data_thick;
            $_POST['Sensitivity']['data_ripe']  = $result_data_ripe;
            $_POST['Sensitivity']['data_road']  = $result_data_rod;
            $_POST['Sensitivity']['data_mills']  = $result_data_mls;
            $_POST['Sensitivity']['data_town']  = $result_data_twn;

            if (!Yii::$app->user->isGuest) {
                $_POST['Sensitivity']['id_user']    = Yii::$app->user->identity->id;
            }
    ...
    /**
     * Updates an existing Sensitivity model.
    ...
    /**
     * Deletes an existing Sensitivity model.
    ...
    /**
     * Finds the Sensitivity model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Sensitivity the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Sensitivity::findOne($id)) !== null) {
            return $model;
        } else {
